...
public class OrderDataSource extends PageKeyedDataSource<Integer, OrderListResponse> {
    ...
    public OrderDataSource(Context context, OrderListRequest orderListRequest,
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params,
                            @NonNull LoadInitialCallback<Integer, OrderListResponse> callback) {
        if (utility.isNetworkAvailable()) {
            Call<ApiResponse> call = apiInterface.getOrderList(utility.getAuthorizationKey(),
                    utility.getUserId(), utility.getFirebaseToken(), utility.getLangauge(),
                    orderListRequest, FIRST_PAGE, PAGE_SIZE);
                        ...
                        if (apiResponse != null) {
                            if (apiResponse.getCode() == 200) {
                                ...
                                List<OrderListResponse> orderList =
                                    ...
                                    callback.onResult(orderList, 0, FIRST_PAGE + 1);
                                    emptyListInterface.order(orderList.size());
                            } else {
    ...
    public void loadBefore(@NonNull LoadParams<Integer> params,
                           @NonNull LoadCallback<Integer, OrderListResponse> callback) {
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params,
        ...
        if (utility.isNetworkAvailable()) {
            Call<ApiResponse> call = apiInterface.getOrderList(utility.getAuthorizationKey(),
                    utility.getUserId(), utility.getFirebaseToken(), utility.getLangauge(),
                    orderListRequest, params.key, PAGE_SIZE);
            call.enqueue(new Callback<ApiResponse>() {
                @Override
                public void onResponse(@NonNull Call<ApiResponse> call,
                                       @NonNull Response<ApiResponse> response) {
                    if (response.isSuccessful()) {
                        ...
                        if (apiResponse != null) {
                            if (apiResponse.getCode() == 200) {
                                ...
                                List<OrderListResponse> orderList =
                                        gson.fromJson(apiResponse.getData(), listType);
                                callback.onResult(orderList, params.key + 1);
                            } else {
                                errorMessage.postValue(apiResponse.getData().getAsString());
                            }
                        }
